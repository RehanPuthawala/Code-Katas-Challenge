...
const getTurkishNumber = (num) => {
  ...
  if (Math.floor(num / 10) !== 1 && num !== 10) {
    return num >= nineDigitArr.length
      ? `${tensArr[Math.floor(num / 10) - 1]} ${nineDigitArr[num % 10]}`
      : nineDigitArr[num];
  } else if (num === 10) {
    return `${tensArr[0]}`;
  }

  return `${tensArr[0]} ${nineDigitArr[num % 10]}`;
};

console.log(getTurkishNumber(22));
